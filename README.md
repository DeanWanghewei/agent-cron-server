...
```bash
# 确保 agent-cron 目录不存在，避免 cp 产生嵌套
rm -rf ~/.hermes/skills/agent-cron
...
> ⚠️ 如果目标目录已存在，`cp -r skill/ target/` 会把整个 `skill/` 作为子目录拷进去（变成 `target/skill/SKILL.md`），导致 Hermes 无法识别。务必先删除目标目录或使用 `cp -r skill/* target/`。
